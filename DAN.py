...
class Monitor:
    def __init__(self, profiles_path="DAN_profiles/", target_ips=[""], num_trainprobes=-1, probe_interval=-1, rt_plotting=False, window_size = 20, delete_cache=False):
        ...
        if self.config['target_ips'][0][-4:] == ".csv":
            ...
            if os.path.isfile(ipfile):
                ...
                file.close()
                if len(ips) == 0:
                    raise Exception(ipfile +" is empty. Type 'DAN -h' for help")
                self.targetIPs = [ip.rstrip() for ip in ips]
            else:
                raise Exception("Can't find "+ipfile+". Type 'DAN -h' for help")
            ...
            raise Exception("No target IP(s) were provided. Type 'DAN -h' for help")
    ...
    def report(self, status, probe_count):
        ...
        print("DAN Status  --  Runtime: "+ "{:0>8}".format(str(datetime.timedelta(seconds=time.time()-self.start_time))))
        print(table)
...
if __name__ == '__main__':
    ...
    parser.description = description='DAN: A man-in-the-middle detector for LAN environments. By Yisroel Mirsky 2019\nBounces ICMP packet ' \
    ...
    parser.epilog = 'When launched, DAN will monitor the IPv4 addresses in the local file IPs.csv, unless a target IP ' \
                    ...
                    'For more information, please read our paper:\nDAN: Using Echo-Analysis to Detect Man-in-the-Middle ' \
    ...
    parser.add_argument('-f',default="DAN_profiles/",help="load/save profiles from the given directory <F>. If is does not exist, it will be created. \nDefault path is ./DAN_profiles.")
    ...
    parser.add_argument('--reset',action='store_true',help="Deletes the current configuration and all IP profiles stored on disk before initilizing DAN")
